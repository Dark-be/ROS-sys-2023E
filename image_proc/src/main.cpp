...
float angle(cv::Point2f pt1, cv::Point2f pt2) ;
...
bool getRec(cv::Mat input)
{
    cv::imshow("resize", input);
    cv::Mat recGrayImg, recBinaryImg;
    cv::cvtColor(input, recGrayImg, cv::COLOR_BGR2GRAY);
    cv::threshold(recGrayImg, recBinaryImg, 80.0, 255.0, cv::THRESH_BINARY_INV);
    cv::imshow("rectangular", recBinaryImg);
    ...
    centerPoints.resize(4);
    cv::Point2f center;
    for(int i = 0; i < 4; i++)
    {
        centerPoints[i] = (box1[i] + 3*box2[i]) / 4;
        center += centerPoints[i];
    }
    center *= (1. / 4);
    std::sort(centerPoints.begin(), centerPoints.end(), [center](cv::Point2f a, cv::Point2f b) {
        return angle(center, a) < angle(center, b);
    });
    for(int i = 0; i < 4; i++) 
    {
        // std::cout << "rectangular" << i << " : " << center_points[i] << std::endl;
        centerPoints[i].x = (centerPoints[i].x - dstWidth/2) / 10;
        centerPoints[i].y = (centerPoints[i].y - dstHeight/2) / 10;
        pubPoint.data.push_back(centerPoints[i].x);
        pubPoint.data.push_back(centerPoints[i].y);
    }
    return true;
}
